...
function main() {
  ...
  const opportunityKeys = recommendations.map(item => api.RecommendationConstraints.opportunityFamilyKey(item)).filter(Boolean);
  const duplicateOpportunityKeys = opportunityKeys.filter((key, index, keys) => keys.indexOf(key) !== index);
  ...
  assertCheck(report, "no_duplicate_opportunity_families", duplicateOpportunityKeys.length === 0, { duplicateOpportunityKeys, opportunityKeys });
