      ...
      <SectionCard eyebrow="Selection Pool" title="选品池" description="正在载入候选商品。">
        <div className="grid gap-4 lg:grid-cols-3">
          {["正在读取商品", "正在整理指标", "正在准备页面"].map((label) => (
            <div key={label} className="surface-card animate-pulse rounded-[28px] border border-[var(--border)] p-6">
      ...
      {items.length > 0 ? (
        <section className="grid gap-5 lg:grid-cols-2">
          {items.map((item) => (
            <article
              key={item.asin}
              className="surface-card ambient-ring flex flex-col gap-5 rounded-[30px] border border-[var(--border)] p-5"
            >
