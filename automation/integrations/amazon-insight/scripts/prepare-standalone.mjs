...
const pnpmClientRoot = join(root, "node_modules", ".pnpm");
const pnpmPackages = await readdir(pnpmClientRoot).catch(() => []);
const generatedClientPackage = pnpmPackages.find((entry) => entry.startsWith("@prisma+client@"));
if (generatedClientPackage) {
  const generatedClient = join(pnpmClientRoot, generatedClientPackage, "node_modules", ".prisma", "client");
  await copyIfExists(generatedClient, join(root, "node_modules", ".prisma", "client"));
  await copyIfExists(generatedClient, join(standaloneRoot, "node_modules", ".prisma", "client"));
}
