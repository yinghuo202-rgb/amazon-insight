...
import type { ProductCatalogItem, ProfitabilityData } from "@/lib/inventory/contracts";
...
export function SkuDetailDashboard({ dashboard, profitability, product, sku, canceledOrders, purchaseOrders, shipmentHistory }: { dashboard: SkuDetailViewModel; profitability: ProfitabilityData["rows"][number] | null; product: ProductCatalogItem | null; sku: string; canceledOrders: PurchaseOrderReview[]; purchaseOrders: SkuPurchaseOrderDetail[]; shipmentHistory: ShipmentHistoryItem[] }) {
  ...
  const latestRevenue = profitability ? formatCurrency(profitability.productSales) : "—";
  const latestProfit = profitability ? formatCurrency(profitability.actualProfit) : "—";
