...
export function PurchaseOrderBrowser({ orders }: { orders: PurchaseOrderListItem[] }) {
  ...
  const { visible, hasMore, sentinelRef } = useInfiniteRows(filtered, pageSize);
  ...
  return <div className="space-y-4">
    ...
    <OpsCard>
      <div className="flex flex-col gap-3 border-b border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-1 bg-slate-100 p-1">{(["OPEN", "COMPLETED", "ALL"] as const).map((value) => <button key={value} type="button" onClick={() => setStatus(value)} className={`px-3 py-1.5 text-xs font-semibold ${status === value ? "bg-white text-emerald-700 shadow-sm" : "text-slate-500"}`}>{value === "OPEN" ? "待催货" : value === "COMPLETED" ? "已完成" : "全部订单"}</button>)}</div><label className="relative"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索订单号、SKU、供应商" className="w-full border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm outline-none focus:border-emerald-600 sm:w-80" /></label></div>
      ...
      <div ref={sentinelRef} className="border-t border-slate-100 px-4 py-3 text-center text-xs text-slate-500">显示 {visible.length} / {filtered.length} 单 · {hasMore ? "继续下滑加载" : "已显示全部"}</div>
    </OpsCard>
  </div>;
}
