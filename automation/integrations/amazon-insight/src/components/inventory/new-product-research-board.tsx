...
import { calculateResearchCandidate, RESEARCH_RMB_PER_USD, type ResearchCandidate, type ResearchCandidateInput } from "@/lib/inventory/new-product-research";

const number = new Intl.NumberFormat("zh-CN");
const money = (value: number | null) => value === null ? "—" : `$${value.toFixed(2)}`;
const emptyCandidate: ResearchCandidateInput = { sku: "", name: "", amazonPrice: null, firstMile: null, storageFee: null, commission: null, orderFee: null, importDutyRate: null, purchaseCostRmb: null, competitorUrl: "" };
...
export function NewProductResearchBoard({ data }: { data: NewProductResearchData }) {
  ...
  function openEdit(item: ResearchCandidate) {
    ...
    setDraft({ sku: item.sku, name: item.name, amazonPrice: item.amazonPrice, firstMile: item.firstMile, storageFee: item.storageFee, commission: item.commission, orderFee: item.orderFee, importDutyRate: item.importDutyRate, purchaseCostRmb: item.purchaseCostRmb, competitorUrl: item.competitorUrl });
    setMessage("");
  }
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(320px,.65fr)]">
      <OpsCard>
        ...
        <div className="overflow-x-auto"><table className="min-w-[820px] w-full text-left text-xs"><thead className="bg-slate-50 text-[10px] uppercase tracking-[0.12em] text-slate-500"><tr><th className="px-5 py-3">SKU / 产品</th><th className="px-3 py-3">售价</th><th className="px-3 py-3">采购成本</th><th className="px-3 py-3">毛利</th><th className="px-3 py-3">毛利率</th><th className="px-3 py-3">竞品</th><th className="px-5 py-3 text-right">操作</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map((item) => <tr key={item.sku} className="hover:bg-slate-50/80"><td className="px-5 py-3 font-semibold text-slate-900">{item.sku}<span className="mt-0.5 block max-w-64 truncate text-[10px] font-normal text-slate-400" title={item.name}>{item.name}</span></td><td className="px-3 py-3 text-slate-700">{money(item.amazonPrice)}</td><td className="px-3 py-3 text-slate-700">{item.purchaseCostRmb === null ? "—" : `¥${item.purchaseCostRmb.toFixed(2)}`}</td><td className={`px-3 py-3 ${item.grossProfit !== null && item.grossProfit < 0 ? "text-rose-700" : "text-slate-700"}`}>{money(item.grossProfit)}</td><td className="px-3 py-3">{item.grossMargin === null ? <OpsBadge>待补成本</OpsBadge> : <OpsBadge tone={item.grossMargin >= .3 ? "emerald" : item.grossMargin >= 0 ? "amber" : "rose"}>{(item.grossMargin * 100).toFixed(1)}%</OpsBadge>}</td><td className="px-3 py-3">{item.competitorUrl ? <a href={item.competitorUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-blue-700 hover:text-blue-900">查看 <ExternalLink className="h-3 w-3" /></a> : <span className="text-slate-400">—</span>}</td><td className="px-5 py-3 text-right"><button type="button" onClick={() => openEdit(item)} className="inline-flex items-center gap-1 text-emerald-700 hover:underline"><Pencil className="h-3 w-3" />编辑</button></td></tr>)}</tbody></table></div>
        {!filtered.length ? <p className="p-10 text-center text-xs text-slate-400">没有符合搜索条件的候选产品。</p> : null}
        <div className="border-t border-slate-100 px-5 py-3 text-xs text-slate-500">显示 {filtered.length} / {candidates.length} 个候选</div>
      </OpsCard>
      <OpsCard>
