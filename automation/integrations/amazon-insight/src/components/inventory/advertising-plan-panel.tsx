...
export function AdvertisingPlanPanel({ market, period, rows, currency }: { market: "US" | "CA"; period: string | null; rows: CalculatedCampaign[]; currency: string }) {
  ...
  const pageSize = 25; const pageCount = Math.max(1, Math.ceil(filtered.length / pageSize)); const safePage = Math.min(page, pageCount); const visible = filtered.slice((safePage - 1) * pageSize, safePage * pageSize);
  ...
  return <OpsCard>
    ...
    <div className="flex flex-col gap-2 border-t border-slate-100 p-4 sm:flex-row sm:items-center sm:justify-between"><label className="relative w-full sm:max-w-sm"><span className="sr-only">搜索广告调整草稿</span><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="搜索活动、SKU 或备注" className="w-full border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm" /></label><select aria-label="广告草稿动作" value={actionFilter} onChange={(event) => { setActionFilter(event.target.value); setPage(1); }} className="border border-slate-200 bg-white px-3 py-2 text-sm"><option value="ALL">全部调整动作</option>{[...new Set(drafts.map((item) => item.recommendedAction))].map((action) => <option key={action} value={action}>{advertisingActionLabels[action]}</option>)}</select></div>
