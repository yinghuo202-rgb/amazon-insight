    ...
    >
      <div className="grid gap-5 lg:grid-cols-2">
        {cases.map((item) => (
          <article
            key={item.analysisId}
            className="surface-strong ambient-ring overflow-hidden rounded-2xl border border-[var(--border)]"
          >
            <div className="grid gap-5 p-5 md:grid-cols-[120px_minmax(0,1fr)] md:p-6">
              ...
              <img
                src={item.imageUrl ?? ""}
                alt={item.title}
                className="h-[120px] w-[120px] rounded-xl border border-[var(--border)] bg-slate-100 object-cover"
              />
